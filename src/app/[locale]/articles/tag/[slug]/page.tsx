...
export default function TagPage() {
  ...
  const articles = allArticles.filter(article =>
    article.tags.some(({ tag: articleTag }) => articleTag.id === tag.id)
  );
  ...
  return (
    <>
      <Head>
        <title>{`${tag.name} Articles`} | KayanLive</title>
        <meta name="description" content={`Explore articles tagged with ${tag.name} from KayanLive`} />
        <meta property="og:title" content={`${tag.name} Articles`} />
        <meta property="og:description" content={`Explore articles tagged with ${tag.name} from KayanLive`} />
        <meta property="og:type" content="website" />
        <link rel="canonical" href={`https://kayanlive.com/${locale}/articles/tag/${tagSlug}`} />
      </Head>
        ...
        <div className="max-w-7xl mx-auto px-4 py-12">
          ...
          >
            <nav className="flex items-center gap-2 text-sm text-[#888888] mb-6" style={{ fontFamily: '"Poppins", sans-serif' }}>
              ...
              <ChevronRight size={16} />
              <span className="text-[#7afdd6]">#{tag.name}</span>
            </nav>
          </motion.div>
          ...
          >
            <div className="flex items-center justify-center gap-3 mb-6">
              <div className="p-3 rounded-[15px]" style={{ background: 'linear-gradient(90deg, #7afdd6 0%, #b8a4ff 100%)' }}>
                <Tag size={32} className="text-[#2c2c2b]" />
              </div>
            </div>
            ...
            <div className="flex items-center justify-center gap-6 text-sm text-[#888888]" style={{ fontFamily: '"Poppins", sans-serif' }}>
              ...
              <span>
                {articlesData?.total || 0} {t('tags.articlesWithTag')}
              </span>
            </div>
          </motion.div>
          ...
          >
            {isLoading ? (
              ...
              <div className="text-center py-16">
                <div className="mb-6">
                  <Tag size={64} className="mx-auto text-[#888888] opacity-50" />
                </div>
                ...
                <p className="text-[#888888] mb-8" style={{ fontFamily: '"Poppins", sans-serif' }}>
                  No articles found with this tag and the current filters.
                </p>
                <button
                  onClick={resetFilters}
