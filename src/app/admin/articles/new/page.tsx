...
export default function NewArticlePage() {
  ...
  // Set document title
  useEffect(() => {
    document.title = 'New Article - KayanLive Admin';
    return () => {
  ...
  const createArticleMutation = api.article.create.useMutation({
    ...
    onError: (error) => {
      addToast(error.message || 'Failed to create article', 'error');
    },
  });
  ...
  const handleSave = (status: ArticleStatus = 'DRAFT') => {
    ...
    // Content creators can only save as DRAFT or submit for review
    let finalStatus = status;
    if (userRole === 'CONTENT_CREATOR') {
      finalStatus = status === 'PENDING_REVIEW' ? 'PENDING_REVIEW' : 'DRAFT';
    }
    ...
    createArticleMutation.mutate({
      ...
      locale: formData.locale as 'en' | 'ar' | 'fr' | 'zh' | 'ru',
      status: finalStatus,
      type: formData.type,
      publishScheduleType: formData.publishScheduleType,
      scheduledAt: formData.publishScheduleType === 'SCHEDULED' && formData.scheduledAt ? formData.scheduledAt : undefined,
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[#888888] text-lg">
              Create a new article for your website
            </p>
          </div>
        </div>
