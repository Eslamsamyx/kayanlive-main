        ...
        >
          ...
          <div className="absolute left-0 md:left-[137px] top-20 md:top-20 w-full md:w-[567px] flex flex-col gap-[30px] md:gap-[42px] items-center md:items-start justify-start z-10 px-8 md:px-0">
            <h3 className="text-white text-[30px] md:text-[50px] leading-[30px] md:leading-[51px] capitalize text-center md:text-left max-w-[290px] md:max-w-none md:w-auto">
              Looking to collaborate on future events, activations, or long-term initiatives?
            </h3>
            
            {/* CTA Button with Arrow - Redesigned for proper height matching */}
            <div 
              className="flex items-stretch gap-0 mx-4 md:mx-0"
              onMouseEnter={() => setIsHovered(true)}
              onMouseLeave={() => setIsHovered(false)}
            >
              <button className="bg-white rounded-[900px] px-3 md:px-[25px] py-3 md:py-[18px] flex items-center gap-2 md:gap-3 flex-1 max-w-[240px] md:max-w-none">
                <span className="text-[#2c2c2b] text-[14px] md:text-[20px] capitalize text-center leading-tight">Explore a Strategic Collaboration</span>
              </button>
              <div 
                className="bg-white rounded-full flex items-center justify-center transition-all duration-300 flex-shrink-0"
                style={{
                  width: 'auto',
                  aspectRatio: '1',
